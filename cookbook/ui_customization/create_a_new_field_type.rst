...
If you want a custom rendering for one of your attribute, attribute type or reference data you need to create a new field type. In this cookbook, we will got through each steps needed to create a custom field type.
...
Before diving into code, we need to understand what's going on under the hood:
 ...
 - This field provider gets an attribute and return a field type
...
Your field provider is now registred congrats !
...
Then, last opperation, match the field type (`acme-range-field`) with the require module (`acme/range-field`):
